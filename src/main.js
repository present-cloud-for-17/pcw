...
//axios.defaults.headers.ContenType = "application/x-www-form-urlencoded";
Vue.prototype.qs = qs;
...
router.beforeEach((to, from, next) => {
    ...
    const role = localStorage.getItem('ms_status');//权限判断
    const secret = 1;//为token验证提供判断，每次判断前需向端发送验证请求，返回判断结果，待完善

    if (secret !== 1 && to.path !== '/login' && to.path !== '/register' && to.path !== '/registerfinal' && to.path !== '/forgetpassword') {
        //无token情况
        next('/login');
        document.title = '登录';
    } else if(!user && to.path !== '/login' && to.path !== '/register' && to.path !== '/registerfinal' && to.path !== '/forgetpassword') {
        //有token非用户情况
        next('/login');
        document.title = '登录';
